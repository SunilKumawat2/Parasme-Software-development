...
const AdminOurServicesPPC = () => {
        // This Offcanvas for the Add PPC 
        const [show, setShow] = useState(false);
        const handleClose = () => setShow(false);
        const handleShow = () => setShow(true);
    
        // This Offcanvas for the Edit PPC
        const [showEdit, setShowEdit] = useState(false);
        const handleCloseEdit = () => setShowEdit(false);
        const handleShowEdit = () => setShowEdit(true);
    
        // This Usestate use for the Add The PPC Data 
        ...
        // The useState Use for the All PPC Show 
        const [OurServicesPPC, setOurServicesPPC] = useState([]);
    
        // The useState Use for the All Awards Iamges Show By ID
        const [OurServicesPPCById, setOurServicesPPCById] = useState({});
        const { _id } = useParams();
        useEffect(() => {
            GetOurServicesPPC();
            GetOurServicesPPCById(_id);
        }, [_id])
    
        // This Get Methods for the GetImages PPC
        const GetOurServicesPPC = async () => {
            await axios.get(`${API_BASE_URL}/OurServicesPPC/get`).then((response) => {
                setOurServicesPPC(response.data.data);
            }).catch((error) => {
                console.log(error);
            })
        }
    
        //   This Post Methods for the Create PPC
        const HandleSubmit = (e) => {
            ...
            axios.post(`${API_BASE_URL}/OurServicesPPC/post`, formData).then((response => {
        ...
        // This Delete Methods for the Delete PPC
        const deleteSEOS = async (_id) => {
            try {
                const response = await axios.delete(`${API_BASE_URL}/OurServicesPPC/delete/${_id}`);
                console.log(response.data);
                GetOurServicesPPC();
            } catch (error) {
                console.error(error);
        ...
        // This Methods show the PPC By ID
        const GetOurServicesPPCById = async (_id) => {
           await axios.get(`${API_BASE_URL}/OurServicesPPC/getById/${_id}`).then((response) => {
                setOurServicesPPCById(response.data.data)
            }).catch((error) => {
                console.log(error);
        ...
        // This methods for the Update the OurServicesPPC
        ...
        //  This function use for the Edit the PPC
        const EditOurServicesPPC = async (_id) => {
            ...
            if (editFormData.images || editFormData.title || editFormData.description
                ...
                try {
                    const response = await axios.put(`${API_BASE_URL}/OurServicesPPC/update/${_id}`, formData);
                    console.log(response.data);
    
                    // After successfully editing, update the PPC data
                    GetOurServicesPPC();
                } catch (error) {
                    console.error(error);
                }
        ...
        useEffect(() => {
            ...
            if (OurServicesPPCById) {
                setEditFormData({
                    images: OurServicesPPCById.images || "",
                    title: OurServicesPPCById.title || "",
                    description: OurServicesPPCById.description || "",
                    sub_description: OurServicesPPCById.sub_description || "",
                    sub_title: OurServicesPPCById.sub_title || "",
                    about_page: OurServicesPPCById.about_page || "",
                });
            }
        }, [_id, OurServicesPPCById]);
                ...
                <Box component="main" sx={{ flexGrow: 1, p: 3 }} style={{
                    ...
                    <div>
                        <div>
                                ...
                                <Offcanvas.Body>
                                    <form onSubmit={HandleSubmit} method='post' encType='multipart/form-data'
                                        ...
                                        <button className='btn btn AdminUploads mt-5' onHide={handleClose}
                                            type='submit'>Upload our PPC</button>
                                    </form>
                                </Offcanvas.Body>
                            </Offcanvas>
                            ...
                            <Offcanvas show={showEdit} onHide={handleCloseEdit} className="EditSEOSection"
                                ...
                                <Offcanvas.Header className='bg AdminOffcanvas text-white' closeButton>
                                    <Offcanvas.Title >Edit our Services PPC</Offcanvas.Title>
                                </Offcanvas.Header>
                                <Offcanvas.Body>
                                    <form
                                        onSubmit={(e) => {
                                            e.preventDefault();
                                            if (OurServicesPPCById && OurServicesPPCById._id) {
                                                EditOurServicesPPC(OurServicesPPCById._id);
                                            } else {
                                    ...
                                    >
                                        ...
                                        <div className='EditAllPortFoliosInput'>
                                            <div className='EditAllPortFoliosInputInput'>
                                                <img
                                                    src={`${IMG_BASE_URL}${editFormData.images || OurServicesPPCById?.images}`}
                                                    alt='Slider'
                                                    style={{
                                                        width: "180px",
                                                        height: "100px",
                        ...
                        <Table striped bordered hover>
                            ...
                            <tbody>
                                {OurServicesPPC?.map((OurServicesPPCResult, index) => {
                                    return (
                                        <>
                                            <tr>
                                                <td >{index + 1}</td>
                                                <img src={`${IMG_BASE_URL}${OurServicesPPCResult?.images}`}
                                                    alt='' style={{ width: "100px", height: "70px" }} />
                                                <td>{OurServicesPPCResult?.title}</td>
                                                <td>{OurServicesPPCResult?.description?.slice(0, 70)}...</td>
                                                <td>{OurServicesPPCResult?.sub_title}</td>
                                                <td>{OurServicesPPCResult?.sub_description?.slice(0, 50)}...</td>
                                                <td dangerouslySetInnerHTML={{ __html: `${OurServicesPPCResult?.about_page?.slice(0, 50)}...` }} />
                                                <td>
                                                    <MdDelete style={{
                                                        cursor: "pointer", fontSize: '25px'
                                                        , color: "red", margin: "10px auto"
                                                    }}
                                                        onClick={() => deleteSEOS(OurServicesPPCResult?._id)} /></td>
                                                <td>
                                                    <LiaEdit variant="primary" onHide={handleCloseEdit}
                                                        className='mb-4'
                                                        onClick={() => {
                                                            handleShowEdit();
                                                            GetOurServicesPPCById(OurServicesPPCResult?._id);
                                                        }}
                                                        style={{
                                                            cursor: "pointer", fontSize: '25px'
...
export default AdminOurServicesPPC