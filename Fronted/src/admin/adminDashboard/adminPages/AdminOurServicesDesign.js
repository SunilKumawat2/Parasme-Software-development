...
const AdminOurServicesDesign = () => {
      ...
      const [OurServicesdesign, setOurServicesdesign] = useState([]);
  
      // The useState Use for the All Awards Iamges Show By ID
      const [OurServicesdesignById, setOurServicesdesignById] = useState({});
      const { _id } = useParams();
      useEffect(() => {
          GetOurServicesdesign();
          GetOurServicesdesignById(_id);
      }, [_id])
  
      // This Get Api Methods for the GetImages Slider
      const GetOurServicesdesign = async () => {
          await axios.get(`${API_BASE_URL}/OurServicesWebDesign/get`).then((response) => {
              setOurServicesdesign(response.data.data);
          }).catch((error) => {
              console.log(error);
          })
      }
      ...
      const HandleSubmit = (e) => {
          ...
          axios.post(`${API_BASE_URL}/OurServicesWebDesign/post`, formData).then((response => {
      ...
      const deleteSEOS = async (_id) => {
          try {
              const response = await axios.delete(`${API_BASE_URL}/OurServicesWebDesign/delete/${_id}`);
              console.log(response.data);
              GetOurServicesdesign();
          } catch (error) {
              console.error(error);
      ...
      const GetOurServicesdesignById = async (_id) => {
         await axios.get(`${API_BASE_URL}/OurServicesWebDesign/getById/${_id}`).then((response) => {
              setOurServicesdesignById(response.data.data)
          }).catch((error) => {
              console.log(error);
      ...
      // This methods for the Update the OurServicesdesign
      ...
      const EditOurServicesDesign = async (_id) => {
          ...
          if (editFormData.images || editFormData.title || editFormData.description
              ...
              try {
                  const response = await axios.put(`${API_BASE_URL}/OurServicesWebDesign/update/${_id}`, formData);
                  console.log(response.data);
      ...
      useEffect(() => {
          ...
          if (OurServicesdesignById) {
              setEditFormData({
                  images: OurServicesdesignById.images || "",
                  title: OurServicesdesignById.title || "",
                  description: OurServicesdesignById.description || "",
                  sub_description: OurServicesdesignById.sub_description || "",
                  sub_title: OurServicesdesignById.sub_title || "",
                  about_page: OurServicesdesignById.about_page || "",
              });
          }
      }, [_id, OurServicesdesignById]);
                ...
                <Box component="main" sx={{ flexGrow: 1, p: 3 }} style={{
                    ...
                    <div>
                        <div>
                            ...
                            <Offcanvas show={show} onHide={handleClose} className="AddSEOSection">
                                <Offcanvas.Header className='AdminOffcanvas' closeButton>
                                    <Offcanvas.Title>Add our Services Design</Offcanvas.Title>
                                </Offcanvas.Header>
                                <Offcanvas.Body>
                                    <form onSubmit={HandleSubmit} method='post' encType='multipart/form-data'
                                        ...
                                        <button className='btn btn AdminUploads mt-5' onHide={handleClose}
                                            type='submit'>Upload our Design</button>
                                    </form>
                                </Offcanvas.Body>
                            </Offcanvas>
                            ...
                            <Offcanvas show={showEdit} onHide={handleCloseEdit} className="EditSEOSection"
                                ...
                                <Offcanvas.Header className='bg AdminOffcanvas text-white' closeButton>
                                    <Offcanvas.Title >Edit our Services Design</Offcanvas.Title>
                                </Offcanvas.Header>
                                <Offcanvas.Body>
                                    <form
                                        onSubmit={(e) => {
                                            e.preventDefault();
                                            if (OurServicesdesignById && OurServicesdesignById._id) {
                                                EditOurServicesDesign(OurServicesdesignById._id);
                                            } else {
                                    ...
                                    >
                                        ...
                                        <div className='EditAllPortFoliosInput'>
                                            <div className='EditAllPortFoliosInputInput'>
                                                <img
                                                    src={`${IMG_BASE_URL}${editFormData.images || OurServicesdesignById?.images}`}
                                                    alt='Slider'
                                                    style={{
                                                        width: "180px",
                                                        height: "100px",
                        ...
                        <Table striped bordered hover>
                            ...
                            <tbody>
                                {OurServicesdesign?.map((OurServicesdesignResult, index) => {
                                    return (
                                        <>
                                            <tr>
                                                <td >{index + 1}</td>
                                                <img src={`${IMG_BASE_URL}${OurServicesdesignResult?.images}`}
                                                    alt='' style={{ width: "100px", height: "70px" }} />
                                                <td>{OurServicesdesignResult?.title}</td>
                                                <td>{OurServicesdesignResult?.description?.slice(0, 70)}...</td>
                                                <td>{OurServicesdesignResult?.sub_title}</td>
                                                <td>{OurServicesdesignResult?.sub_description?.slice(0, 50)}...</td>
                                                <td dangerouslySetInnerHTML={{ __html: `${OurServicesdesignResult?.about_page?.slice(0, 50)}...` }} />
                                                <td>
                                                    <MdDelete style={{
                                                        cursor: "pointer", fontSize: '25px'
                                                        , color: "red", margin: "10px auto"
                                                    }}
                                                        onClick={() => deleteSEOS(OurServicesdesignResult?._id)} /></td>
                                                <td>
                                                    <LiaEdit variant="primary" onHide={handleCloseEdit}
                                                        className='mb-4'
                                                        onClick={() => {
                                                            handleShowEdit();
                                                            GetOurServicesdesignById(OurServicesdesignResult?._id);
                                                        }}
                                                        style={{
                                                            cursor: "pointer", fontSize: '25px'
...
export default AdminOurServicesDesign